Shader "DissolverShader/DissolveShader" {
	Properties{
		...
		_DissolveEmission("DissolveEmission", Range(0,10)) = 1
		...
		SubShader{
			...
			void surf(Input IN, inout SurfaceOutputStandard o) {
				...
				fixed4 c = tex2D(_MainTex, IN.uv_MainTex + float2(_Offset % 1.0,0)) * _Color;
				fixed4 mask = tex2D(_DissolveMap, IN.uv_DissolveMap);

					if (_DissolveAmount > 0 && mask.r < _DissolveAmount)
						discard;

					o.Albedo = c.rgb;

					if (_DissolveAmount > 0 && mask.r < _DissolveAmount + _DissolveWidth) {
